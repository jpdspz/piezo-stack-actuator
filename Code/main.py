...
from matrix import build_matrix, mu, S, L
...
if __name__ == "__main__":
    ...
    omegas = np.linspace(1, 6000, 2000)
    dets = [det_omega(w) for w in omegas]
    plt.figure(figsize=(10, 6))
    plt.plot(omegas, dets)
    plt.axhline(y=0, color='k', linestyle='--')
    plt.xlabel(r'$\omega$ (rad/s)')
    plt.ylabel(r'$\det(\mathbf{M}(\omega))$')
    plt.title('Determinante vs frequência natural')
    plt.grid(True)
    plt.show()